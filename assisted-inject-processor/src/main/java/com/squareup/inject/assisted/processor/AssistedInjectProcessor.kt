...
class AssistedInjectProcessor : AbstractProcessor() {
  ...
  override fun process(annotations: Set<TypeElement>, roundEnv: RoundEnvironment): Boolean {
    ...
    (typeWrappersWithFactories + typeWrappersWithAssisted)
        ...
        .forEach {
          try {
            parseRequest(it).brewJava().writeTo(filer)
  ...
  private fun parseRequest(type: TypeElement): AssistedInjectRequest {
    ...
    return AssistedInjectRequest(type, factoryType, factoryMethod, bindingKeys, providedKeys,
        assistedKeys)
  }

  private data class AssistedInjectRequest(
      val type: TypeElement,
      val factoryType: TypeElement,
      val factoryMethod: ExecutableElement,
      val bindingKeys: List<BindingKey>,
      val providedKeys: List<BindingKey>,
      val assistedKeys: List<BindingKey>
  )

  private fun AssistedInjectRequest.brewJava(): JavaFile {
    ...
    return JavaFile.builder(generatedName.packageName(), generatedSpec)
